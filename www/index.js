...
async function init() {
  ...
  let versions = await fetch(config.image_url + "/.versions.json", { cache: "no-cache" })
    .then((obj) => { return obj.json(); })
    .then((obj) => { return obj.versions_list.filter((version) => version.localeCompare("19.07.4") >= 0);});
